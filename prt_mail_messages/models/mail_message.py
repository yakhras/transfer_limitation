...
class MailMessage(models.Model):
    ...
    subject_display = fields.Html(compute="_compute_subject_display")
    ...
    def _compute_subject_display(self):
        ...
        for rec in self.with_context(bin_size=False):
            ...
            if rec.starred:
                notification_icons = (
                    '{} &nbsp;<i class="fa fa-star" title="{}"></i>'.format(
                        notification_icons, _("Starred")
                    )
                )
            if rec.has_error > 0:
                notification_icons = (
                    '{} &nbsp;<i class="fa fa-exclamation" title="{}"></i>'.format(
            ...
            if rec.cx_edit_uid:
                notification_icons = (
                    f'{notification_icons} &nbsp;<i class="fa fa-edit"'
                    ' style="color:#1D8348;"'
                    f' title="{rec.cx_edit_message}"></i>'
                )
            # .. attachments
            if rec.attachment_count > 0:
                notification_icons = (
                    '{} &nbsp;<i class="fa fa-paperclip" title="{}"></i>'.format(
                        notification_icons,
    ...
    def reply_prep_context(self):
        ...
        if wizard_mode in ["quote", "forward"]:
            ...
            body = (
                ...
                " Date: {msg_date} <br/>"
                " From: {author} <br/>"
                " Subject: {subject} <br/><br/>{body}</blockquote>"
            ).format(
        ...
        ctx = {
            ...
            if self.author_allowed_id and wizard_mode != "forward"
            else [],  # noqa
