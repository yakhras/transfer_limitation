...
class CrmTeam(models.Model):
    ...
    # Time period-specific fields for totals
    ...
    def _get_date_range(self, weeks=0, days=0):
        ...
        start_date = (today + date_utils.relativedelta(weeks=weeks, days=days)).strftime('%Y-%m-%d')
    ...
    def _compute_unpaid_invoice_totals(self):
        ...
        for team in self:
            ...
            month_ago, today = self._get_date_range(weeks=-4)
            invoices = self.env['unpaid.invoice'].search([('due_date', '>=', month_ago), ('due_date', '<', today), ('team_id', '=', team.id)])
