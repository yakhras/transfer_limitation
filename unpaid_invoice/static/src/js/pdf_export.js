...
ListController.include({
   ...
   this._super.apply(this, arguments);
    ...
    action_def: function () {
        var self = this;
        var fields = this._getExportDialogWidget().defaultExportFields;
        let exported_fields = fields.map(field => ({
            name: field,
            label: this._getExportDialogWidget().record.fields[field].string,
        }));
        let groupedBy = this.renderer.state.groupedBy;
        ...
        ajax.jsonRpc('/get_data', 'call', {
            ...
            // Step 2: Send email with the PDF attachment
            return ajax.jsonRpc('/send_email_unpaid_invoices', 'call', {
                'template_id': 'unpaid_invoice.unpaid_invoice',
                'report_name': groupedBy[0] ? 'unpaid_invoice.export_in_pdf_group_by' : 'unpaid_invoice.export_in_pdf',
                'records': records
            });
            ...
            if (email_response.success) {
                ...
                console.error('Failed to send email:', email_response.error);
            }
        });
    }
