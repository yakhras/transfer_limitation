...
class Users(models.Model):
    ...
    def get_partner_ids(self):
        ...
        employees = self.env['hr.employee'].search([('parent_id.user_id', '=', current_user.id)]).mapped('user_id').ids
        

        # Retrieve partner IDs of the managed employees
        direct_partners = self.env['res.partner'].search([('user_id', '=', employees)])
