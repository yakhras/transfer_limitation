...
### New ###
    ...
    def _get_company_filter_condition(self):
        """Get the company filter condition for multi-company support."""
        return """
            AND (
                sml.company_id IS NULL OR 
                sml.company_id = sw_out.company_id OR 
                sml.company_id = sw_in.company_id
            )
            AND (
                sm.company_id IS NULL OR 
                sm.company_id = sw_out.company_id OR 
    ...
    def _get_internal_to_internal_out_query(self):
        ...
        """.format(
            operation_case=self._get_operation_case_statement(),
            company_filter=self._get_company_filter_condition()
        )
