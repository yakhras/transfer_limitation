...
class StockProductFlowReport(models.Model):
    ...
    stock_balance = fields.Float(string="Stock Balance", readonly=True, group_operator='max')
    ...
    def _get_company_filter_condition_out(self):
        """Get the company filter condition for outbound queries."""
        return """
            AND (
                sml.company_id IS NULL OR 
                sml.company_id = sw_out.company_id
            )
            AND (
                sm.company_id IS NULL OR 
                sm.company_id = sw_out.company_id
            )
        """

    def _get_company_filter_condition_in(self):
        """Get the company filter condition for inbound queries."""
        return """
            AND (
                sml.company_id IS NULL OR 
                sml.company_id = sw_in.company_id
            )
            AND (
                sm.company_id IS NULL OR 
                sm.company_id = sw_in.company_id
            )
        """

    def _get_company_filter_condition_external(self):
        """Get the company filter condition for external queries."""
        return """
            AND (
                (sl.usage = 'internal' AND sml.company_id IS NOT NULL AND sml.company_id = sw_out.company_id) OR
                (sld.usage = 'internal' AND sml.company_id IS NOT NULL AND sml.company_id = sw_in.company_id) OR
                (sl.usage != 'internal' AND sld.usage != 'internal')
            )
            AND (
                (sl.usage = 'internal' AND sm.company_id IS NOT NULL AND sm.company_id = sw_out.company_id) OR
                (sld.usage = 'internal' AND sm.company_id IS NOT NULL AND sm.company_id = sw_in.company_id) OR
                (sl.usage != 'internal' AND sld.usage != 'internal')
            )
        """
